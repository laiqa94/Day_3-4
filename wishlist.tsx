...
import { Angry, Eye, Trash2 } from "lucide-react";
import { inter, poppins } from "../components/font";
import{addToCart,removeFromWishlist,getWishlistItems,moveAllToCart,}from "../../app/actions/page";
...
export default function WishlistPage() {
      ...
      <div className="relative aspect-square bg-[#F5F5F5] rounded-sm mb-4">
        ...
        {isWishlistItem ? (
          <button
            ...
            onClick={() => handleRemoveFromWishlist(item.Product._id.toString())}
          >
  ...
  return (
    <div className={`${inter} min-h-screen bg-white`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex justify-between items-center mb-10">
          <h1 className={`${poppins} text-2xl font-medium`}>
        ...
        {wishlistItems.length > 0 ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-20">
            {wishlistItems.map((item) => (
              <ProductCard key={item.ProductName} item={item} isWishlistItem={true} />
            ))}
          </div>
        ) : (
        ...
        <div className="space-y-10">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h2 className={`${poppins} text-2xl font-medium`}>
                Just For You
              </h2>
            </div>
            <button
